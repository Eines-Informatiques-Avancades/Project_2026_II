...
program main_serial_equil
  ...
  ! 2. Main Monte Carlo Loop (Equilibration)
      ...
      if (mod(istep, geweke_sample_interval) == 0) then
        ...
        if (gbuf_count >= n_geweke .and. mod(gbuf_count, eval_freq) == 0) then
          ...
          if (z_E < z_crit) then
            ...
            write(*,'(A,I2,A,I2,A)') " [Geweke] PASSED (", consec_passes, "/", n_consec, ")"
            if (consec_passes >= n_consec) then
              is_equilibrated = .true.
              write(*,'(A,I10,A)') " *** EQUILIBRIUM DETECTED at step ", istep, " ***"
            end if
          else
            if (consec_passes > 0) write(*,'(A)') " [Geweke] failed - reset"
            consec_passes = 0
          end if
        end if
      end if
  ...
  write(*,'(A)') " ------------------------------------------------------------"
  write(*,'(A)') " [MC Simulation] Equilibrium Reached. Starting Production - (", n_steps, " steps)"
  write(*,'(A)') " ------------------------------------------------------------"

  ! 3. Production Monte Carlo Loop
  close(u_ener)
  close(u_obs)
  close(u_tors)
  close(u_traj)
